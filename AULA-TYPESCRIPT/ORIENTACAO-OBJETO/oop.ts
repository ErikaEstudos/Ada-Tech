import { Produto } from "./classesOOP"
...
class Farmacia extends Estabelecimento implements FarmaciaInterface {
    ...
    public compraRemedioPrescrito(
        ...
        produtosAComprar:string[]): void {
       
        const remediosDisponiveis = this.produtos
        .filter(produto => produtosAComprar.includes(produto.nome))

        if(remediosDisponiveis.length === 0){
            console.log('Infelizmente não temos os remédios em estoque')
        }

         const remediosAutorizados = remediosDisponiveis
        .filter(
            produto => 
...
const farmaciaDoZe = new Farmacia( 
    ...
    [
        ...
        {nome: "remédio controlado 2", valor: 70, receitaObrigatoria: true},
